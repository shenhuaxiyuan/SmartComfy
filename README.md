...
| 截图 1 | 截图 2 | 截图 3 |
|--------|--------|--------|
| ![项目截图 1](ScreenShot/1.png) | ![项目截图 2](ScreenShot/2.png) | ![项目截图 3](ScreenShot/3.png) |
...
4. **访问应用**：
   - 完整权限：打开浏览器访问 `http://localhost:5173`（支持创建/编辑/删除应用）
   - 只读模式：打开浏览器访问 `http://localhost:5174`（仅允许查看和使用应用）
   - 首次运行会自动安装依赖，请耐心等待
...
### Q: 启动后浏览器访问 5173 端口无响应
A:
1. 检查命令行窗口是否有报错
2. 确认 5173 端口未被其他程序占用
...
### v1.0.6
- 优化发布包结构：下载 zip 后双击 start.bat 即可运行
- 服务端代码混淆处理，保护核心业务逻辑
- 生产环境使用 5173（完整权限）/ 5174（只读）端口，与开发环境一致
- 自动检测首次运行，自动安装依赖
